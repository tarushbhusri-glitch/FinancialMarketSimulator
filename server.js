...
/* What each analyst seat is actually calling. Macro, credit and derivatives
   analysts cover markets rather than companies, so their calls cannot be read
   off M.equities. Each cover needs its own current value and its own idea of
   what counts as a move, since 2% of a rate is not comparable to 2% of a share. */
function coverValue(id){
  switch(id){
    case 'rates':            return M.rate5y;
    case 'oil':              return 78.40 + (M.equities.ORC.px - 71.80) * 0.9;
    case 'credit':           return M.creditHY;
    case 'index vol':        return M.indexVol;
    case 'single-name vol':  return EQUITIES.reduce((a,e)=>a + M.equities[e.id].iv, 0) / EQUITIES.length;
    default:                 return M.equities[id] ? M.equities[id].px : null;
  }
}
// a move smaller than this counts as FLAT
const COVER_BAND = { rates:0.025, credit:0.03, 'index vol':0.04, 'single-name vol':0.04 };

function snapshotCovers(){
  const p = Object.fromEntries(EQUITIES.map(e=>[e.id, M.equities[e.id].px]));
  ['rates','oil','credit','index vol','single-name vol'].forEach(k=>{ p[k] = coverValue(k); });
  return p;
}
...
function scoreCalls(prevPx){
  for(const t of Object.values(S.teams)){
    for(const seat of Object.values(t.seats||{})){
      ...
      const before = prevPx[c.cover], after = coverValue(c.cover);
      // never mark a call scored unless it genuinely could be judged, or the
      // terminal ends up reporting "undefined (NaN%)" and docking points for it
      if(before == null || after == null || !isFinite(before) || !isFinite(after) || before === 0) continue;

      const move = (after - before) / Math.abs(before);
      const band = COVER_BAND[c.cover] != null ? COVER_BAND[c.cover] : 0.02;
      const actual = move > band ? 'UP' : move < -band ? 'DOWN' : 'FLAT';
...
function advanceRound(){
  ...
  S.prevPx = snapshotCovers();      // equities AND rates, oil, credit, vols
