...
const os   = require('os');
const { SCENARIOS, CAMPAIGNS, NOISE } = require('./scenarios.js');
...
function intelFor(roleKey, slot, round, teamCode){
  ...
  // a real hint if the coming event touches what this seat covers
  if(next && next.hint){
    const match = next.hint.find(h=>h.r===roleKey && (h.re===cover || covers.length===1));
